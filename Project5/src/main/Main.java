...
public class Main implements Runnable {
	...
	public void run() {
		...
		while (running) {
			...
			if(win.getInput().isKeyPressed(GLFW_KEY_LEFT)) {
				renderer.camera.addPosition(new Vector3f(-10,0,0));
			}
			
			if(win.getInput().isKeyPressed(GLFW_KEY_RIGHT)) {
				renderer.camera.addPosition(new Vector3f(10,0,0));
				
			}
			
			if(win.getInput().isKeyPressed(GLFW_KEY_DOWN)) {
				renderer.camera.addPosition(new Vector3f(0,-10,0));
				
			}
			
			if(win.getInput().isKeyPressed(GLFW_KEY_UP)) {
				renderer.camera.addPosition(new Vector3f(0,10,0));
